...
# # CTG page xpathing #
# ##################
ctg_page_test_url = 'https://www.laithwaites.co.uk/wines/Champagne/_/N-1z141teZ1e?icamp=nav-fw-champagne'
...
def ctg_parsing(fpath, ctg, categories, products):  # TODO : modify xpaths
    ...
    for li in tree.xpath('//div[@class="product-wrapper"]'):
        ...
        products[produrl] = {
            'pdct_name_on_eretailer': clean_xpathd_text(li.xpath('.//h3/a//text()')),
            'volume': clean_xpathd_text(li.xpath('./zzzzzzz')),
            'raw_price': clean_xpathd_text(li.xpath('.//span[@class="price-per-bottle"]/text()')[0:]),
            'raw_promo_price': '',
            'pdct_img_main_url': "".join(li.xpath('.//a//img/@src')[0]),
        }
...
# # KW page xpathing #
###################
search_page_test_url = 'https://www.laithwaites.co.uk/wines/Champagne/_/N-1z141te'
...
def pdct_parsing(fpath, url, products): # TODO : modify xpaths
    ...
    products[url].update({
        'volume': clean_xpathd_text(tree.xpath('(//table[@class="table wine-facts"]//tr)[10]//text()')),
...
kw_search_url = "https://www.laithwaites.co.uk/wines?Ntt={kw}"  # TODO : modify URL
for kw in keywords:
    searches[kw] = []
