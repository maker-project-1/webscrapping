...
# # CTG page xpathing #
# ##################
ctg_page_test_url = 'https://www.abcfws.com/category/GROWLERS/627.uts'
...
def ctg_parsing(fpath, ctg, categories, products):  # TODO : modify xpaths
    ...
    for li in tree.xpath('//div[@class="items-wrapper"]//div[@class="product"]'):
        if not li.xpath('.//a/@href')[0]:
            continue
        produrl = li.xpath('.//a/@href')[0]
        produrl = parse_qs(urlsplit(produrl).query)['url'][0] if 'url' in parse_qs(urlsplit(produrl).query) else produrl
        products[produrl] = {
            'pdct_name_on_eretailer': clean_xpathd_text(li.xpath('.//a[@class="search-product-link"]//span/text()')),
            'volume': clean_xpathd_text(li.xpath('.//div[@class="product-size"]/span/text()')),
            'raw_price': clean_xpathd_text(li.xpath('.//div[@class="product-price"]/span/text()')),
            'raw_promo_price': clean_xpathd_text(li.xpath('.//div[@class="product-price"]/small/text()')),
            'pdct_img_main_url': "".join(li.xpath('./zzzzzzzz')[:1]),
        }
...
# # KW page xpathing #
###################
search_page_test_url = 'https://www.abcfws.com/catalog/search.cmd?form_state=searchForm&keyword=white+wine'
...
def pdct_parsing(fpath, url, products): # TODO : modify xpaths
    ...
    products[url].update({
        ...
        'pdct_img_main_url': clean_url(''.join(''.join(tree.xpath('//div[@class="cc-image-area"]//img/@srcset')[:1]).split(',')[:1]).split('&')[0], root_url),
        'ctg_denom_txt': ' '.join(tree.xpath('//div[contains(@class, "breadcrumb")]//text()')),
    })
    #print(type(products[url]['pdct_img_main_url']))
    return products
...
urls_ctgs_dict = {
    ...
    'vodka': 'https://www.abcfws.com/category/SPIRITS/VODKA/pc/46/47.uts?pageSize=48&currentIndex={start}&pageSize=48',
...
kw_search_url = 'https://www.abcfws.com/searchresults?Ntt={kw}&No=0&Rdm=679&Nr=AND(NOT(product.productTypeId:4),NOT(product.productTypeId:9),NOT(record.type:Store))&N=&Nrpp=96'
for kw in keywords:
    searches[kw] = []
