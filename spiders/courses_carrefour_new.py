...
# # CTG page xpathing #
# ##################
ctg_page_test_url = 'https://courses-en-ligne.carrefour.fr/tous-les-rayons/boissons-et-cave-a-vins/cave-a-vins'
...
def ctg_parsing(fpath, ctg, categories, products):  # TODO : modify xpaths
    ...
    for li in tree.xpath('//div[contains(@class, "cd-Product js-crossBlock")]'):
        if not li.xpath('.//h4/a/@href'):
            continue
        produrl = li.xpath('.//h4/a/@href')[0]
        produrl = parse_qs(urlsplit(produrl).query)['url'][0] if 'url' in parse_qs(urlsplit(produrl).query) else produrl
        products[produrl] = {
            'pdct_name_on_eretailer': clean_xpathd_text(li.xpath('.//h4//text()')),
            'volume': clean_xpathd_text(li.xpath('.//div[@class="cd-ProductDescription"]//text()')),
            'raw_price': clean_xpathd_text(li.xpath('.//div[contains(@class, "cd-ProductPriceUnit")]//text()')),
            'raw_promo_price': clean_xpathd_text(li.xpath('.//span[@class="cd-ProductPricePreDiscount"]//text()')),
...
# # KW page xpathing #
###################
search_page_test_url = 'https://courses-en-ligne.carrefour.fr/search?q=champagne'
...
kw_search_url = "https://courses-en-ligne.carrefour.fr/search?q={kw}"  # TODO : modify URL
for kw in keywords:
    searches[kw] = []
