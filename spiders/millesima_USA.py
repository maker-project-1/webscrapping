...
shop_id = 'millesima_USA'
root_url = 'https://www.millesima-usa.com/'
...
# # CTG page xpathing #
# ##################
ctg_page_test_url = 'https://www.millesima-usa.com/white-wine.html'
...
def ctg_parsing(fpath, ctg, categories, products):  # TODO : modify xpaths
    ...
    for li in tree.xpath('//div[@class="row justify-content-center justify-content-xl-start"]/div'):
        if not li.xpath('.//a/@href'):
            continue
        produrl = li.xpath('.//a/@href')[0]
        produrl = parse_qs(urlsplit(produrl).query)['url'][0] if 'url' in parse_qs(urlsplit(produrl).query) else produrl
        products[produrl] = {
            'pdct_name_on_eretailer': clean_xpathd_text(li.xpath('.//div[@class="title"]/text()')),
            'volume': clean_xpathd_text(li.xpath('.//span[@class="details"]/span/text()')),
            'raw_price': clean_xpathd_text(li.xpath('.//span[@class="offer-price"]/text()')),
            'raw_promo_price': clean_xpathd_text(li.xpath('./zzzzzzzz')),
            'pdct_img_main_url': "".join(li.xpath('./zzzzzzzzzz')),
        }
...
# # KW page xpathing #
###################
search_page_test_url = 'https://www.millesima-usa.com/search?searchTerm=champagne'
...
def pdct_parsing(fpath, url, products): # TODO : modify xpaths
    ...
    products[url].update({
        'volume': clean_xpathd_text(tree.xpath('//span[@class="details"]/span/text()')),
        'pdct_img_main_url': clean_url(''.join(tree.xpath('//div[@class="product-img product-full"]/img/@src')[0]), root_url),
        'ctg_denom_txt': ' '.join(tree.xpath('//ol[@class="breadcrumb-product breadcrumb millesima-breadcrumb"]//text()')),
    })
    return products
...
###################
urls_ctgs_dict = {
    'champagne': 'https://www.millesima-usa.com/champagne.html?filter=&page={page}&sort=1#',
    #    'cognac': '',
    'sparkling': 'https://www.millesima-usa.com/type-sparkling-wine.html?filter=&page={page}&sort=1#',
    #    'vodka': '',
    #    'still_wines': '',
    #    'gin': '',
    #    'tequila': '',
    'red_wine': 'https://www.millesima-usa.com/red-wine.html?filter=&page={page}&sort=1#',
    'white_wine': 'https://www.millesima-usa.com/white-wine.html?filter=&page={page}&sort=1#',
}
...
kw_search_url = "https://www.millesima-usa.com/search?searchTerm={kw}&filter=&page={page}&sort=1#"  # TODO : modify URL
for kw in keywords:
    searches[kw] = []
