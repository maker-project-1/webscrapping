...
def ctg_parsing(fpath, ctg, categories, products):  # TODO : modify xpaths
    ...
    for li in tree.xpath('//li[@data-item-id]'):
        if not li.xpath('.//a[@class="goodsname"]/@href'):
            continue
        ...
        products[produrl] = {
            ...
            'raw_promo_price': clean_xpathd_text(li.xpath('/zzzzz'), unicodedata_normalize=True),
...
def kw_parsing(fpath, kw, searches, products):  # TODO : modify xpaths
    ...
    for li in tree.xpath('//li[@data-item-id]'):
        ...
        products[produrl] = {
            ...
            'volume': clean_xpathd_text(li.xpath('.//a[@class="goodsname"]//text()'), unicodedata_normalize=True),
            'raw_price': clean_xpathd_text(li.xpath('.//p[contains(@class, "goodsprice ")]/text()'), unicodedata_normalize=True),
            'raw_promo_price': clean_xpathd_text(li.xpath('/zzzzzz'), unicodedata_normalize=True),
            'pdct_img_main_url': "".join(li.xpath('.//img/@src')[0]),
        }
        products[produrl]['brnd'] = brm.find_brand(products[produrl]['pdct_name_on_eretailer'])['brand']
        print(products[produrl], produrl)
