...
# # CTG page xpathing #
# ##################
ctg_page_test_url = 'https://www.lagrandeepicerie.com/fr/cave/champagne/'
...
def ctg_parsing(fpath, ctg, categories, products):  # TODO : modify xpaths
    ...
    for li in tree.xpath('//*[@id="search-result-items"]/div'):
        ...
        products[produrl] = {
            ...
            'volume': clean_xpathd_text(li.xpath('.//span[@itemprop="weight"]//text()')),
            'raw_price': clean_xpathd_text(li.xpath('.//span[@class="box-price"]//text()')),
            'raw_promo_price': '',
            'pdct_img_main_url': '',
        }
...
# # KW page xpathing #
###################
search_page_test_url = 'https://www.lagrandeepicerie.com/fr/cave/champagne/'
...
def pdct_parsing(fpath, url, products): # TODO : modify xpaths
    ...
    products[url].update({
        ...
        'pdct_img_main_url': clean_url(''.join(tree.xpath('//div[@class="product-primary-image"]//img[@itemprop="image"]/@src')[:1]), root_url),
        'ctg_denom_txt': ' '.join(tree.xpath('//*[@itemtype="http://schema.org/BreadcrumbList"]//text()')),
    })
    return products
...
kw_search_url = "https://www.lagrandeepicerie.com/fr/votre-recherche?q={kw}&lang=fr&simplesearch=Go"  # TODO : modify URL
for kw in keywords:
    searches[kw] = []
