...
def ctg_parsing(fpath, ctg, categories, products):  # TODO : modify xpaths
    ...
    for li in tree.xpath('//ul[@class="subcat"]/li'):
        ...
        products[produrl] = {
            'pdct_name_on_eretailer': clean_xpathd_text(li.xpath('.//span[@class="name"]//text()')),
            'volume': clean_xpathd_text(li.xpath('.//span[contains(@id, "weight")]/span/text()')),
            'raw_price': clean_xpathd_text(li.xpath('.//*[contains(@id, "original-price")]//text()')),
            'raw_promo_price': clean_xpathd_text(li.xpath('.//*[contains(@id, "current-price")]//text()')),
            'pdct_img_main_url': "".join(li.xpath('.//*[@class="productImage"]/img/@src')),
        }
...
# # KW page xpathing #
###################
search_page_test_url = 'https://www.leshop.ch/fr/search?query=champagne'
...
kw_search_url = "https://www.leshop.ch/en/search?query={kw}"  # TODO : modify URL
for kw in keywords:
    searches[kw] = []
