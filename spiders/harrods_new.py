...
# # CTG page xpathing #
# ##################
ctg_page_test_url = 'https://www.harrods.com/en-gb/food-and-wine/wine-and-spirits/champagne-and-sparkling?icid=megamenu_food-wine_wine-spirits_champagne-sparkling'
...
def ctg_parsing(fpath, ctg, categories, products):  # TODO : modify xpaths
    ...
    for li in tree.xpath('//li[@class="product-grid_item"]'):
        if not li.xpath('.//a[contains(@class, "product-card_link")]/@href'):
            continue
        produrl = li.xpath('.//a[contains(@class, "product-card_link")]/@href')[0]
        produrl = parse_qs(urlsplit(produrl).query)['url'][0] if 'url' in parse_qs(urlsplit(produrl).query) else produrl
        products[produrl] = {
            'pdct_name_on_eretailer': clean_xpathd_text(li.xpath('.//span[@class="product-card_brand"]//text()')),
            'volume': clean_xpathd_text(li.xpath('./zzzzzzzzzzz')),
            'raw_price': '£' + clean_xpathd_text(li.xpath('.//span[@class="price_amount"]/@content')),
...
# # KW page xpathing #
###################
search_page_test_url = 'https://www.harrods.com/en-gb/search?searchTerm=champagne'
...
def kw_parsing(fpath, kw, searches, products):  # TODO : modify xpaths
    ...
    for li in tree.xpath('//li[@class="product-grid_item"]'):
        ...
        products[produrl] = {
            ...
            'raw_price': '£' + clean_xpathd_text(li.xpath('.//span[@class="price_amount"]/@content')),
            'raw_promo_price': clean_xpathd_text(li.xpath('./zzzzzzz')),
...
def pdct_parsing(fpath, url, products): # TODO : modify xpaths
    ...
    products[url].update({
        'volume': clean_xpathd_text(tree.xpath('//span[contains(@id, "productSize")]//text()'), unicodedata_normalize=True),
        'pdct_img_main_url': clean_url(''.join(tree.xpath('//section[@class="pdp_images"]//img/@src')), root_url),
        'ctg_denom_txt': ' '.join(tree.xpath('//section[@class="breadcrumb"]//text()')),
    })
    return products
