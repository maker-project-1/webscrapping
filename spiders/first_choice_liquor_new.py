...
from ers import all_keywords_aus as keywords, fpath_namer, mh_brands, clean_url, shop_inventory_lw_csv
...
# # CTG page xpathing #
# ##################
ctg_page_test_url = 'https://www.firstchoiceliquor.com.au/Sparkling'
...
def ctg_parsing(fpath, ctg, categories, products):  # TODO : modify xpaths
    ...
    for li in tree.xpath('//ul[@class="productList"]/li'):
        ...
        products[produrl] = {
            ...
            'volume': clean_xpathd_text(li.xpath('.//div[@class="product-tile-des"]/a/text()')),
            'raw_price': clean_xpathd_text(li.xpath('.//div[@class="product-tile-price-small"]//span[@class="price"]//text()')).replace(" ", ""),
            'raw_promo_price': clean_xpathd_text(li.xpath('./zzzzzzzzzz')),
            'pdct_img_main_url': "".join(li.xpath('.//div[@class="image_r1_alignment"]//img/@src')),
        }
...
# # KW page xpathing #
###################
search_page_test_url = 'https://www.firstchoiceliquor.com.au/Search?q=champagne'
...
def kw_parsing(fpath, kw, searches, products):  # TODO : modify xpaths
    ...
    for li in tree.xpath('//ul[@class="productList"]/li'):
        ...
        products[produrl] = {
            ...
            'volume': clean_xpathd_text(li.xpath('.//div[@class="product-tile-des"]/a/text()')),
            'raw_price': clean_xpathd_text(li.xpath('.//div[@class="product-tile-price-small"]//span[@class="price"]//text()')).replace(" ", ""),
            'raw_promo_price': clean_xpathd_text(li.xpath('./zzzzzzzzzzz')),
            'pdct_img_main_url': "".join(li.xpath('.//div[@class="image_r1_alignment"]//img/@src')),
        }
        products[produrl]['brnd'] = brm.find_brand(products[produrl]['pdct_name_on_eretailer'])['brand']
        print(products[produrl], produrl)
...
def pdct_parsing(fpath, url, products): # TODO : modify xpaths
    ...
    products[url].update({
        ...
        'pdct_img_main_url': clean_url(''.join(tree.xpath('//div[@class="fullImg"]//img/@src')[:1]), root_url),
...
kw_search_url = "https://www.firstchoiceliquor.com.au/Search?q={kw}&show=500"  # TODO : modify URL
for kw in keywords:
    searches[kw] = []
