...
# # CTG page xpathing #
# ##################
ctg_page_test_url = 'https://www.67wine.com/wines/?item_type=sparkling'
...
def ctg_parsing(fpath, ctg, categories, products):  # TODO : modify xpaths
    ...
    for li in tree.xpath('//ul[@class="products clearfix products-4"]/li'):
        if not li.xpath('.//a/@href'):
            continue
        produrl = li.xpath('.//a/@href')[0]
        produrl = parse_qs(urlsplit(produrl).query)['url'][0] if 'url' in parse_qs(urlsplit(produrl).query) else produrl
        products[produrl] = {
            'pdct_name_on_eretailer': clean_xpathd_text(li.xpath('.//h3[@class="product-title"]/a/text()')),
            'volume': clean_xpathd_text(li.xpath('./zzzzzzz')),
            'raw_price': '$' + clean_xpathd_text(li.xpath('(.//span[@class="woocommerce-Price-amount amount"]/text())[1]')),
            'raw_promo_price': clean_xpathd_text(li.xpath('./zzzzzzzzzz')),
            'pdct_img_main_url': "".join(li.xpath('(.//div[@class="crossfade-images"]//img/@src)[1]')),
        }
        if len(products[produrl]['raw_price']) == 1:
...
# # KW page xpathing #
###################
search_page_test_url = 'https://www.67wine.com/websearch_results.html?kw=champagne'
...
def kw_parsing(fpath, kw, searches, products):  # TODO : modify xpaths
    ...
    for li in tree.xpath('//ul[@class="products clearfix products-4"]/li'):
        ...
        products[produrl] = {
            'pdct_name_on_eretailer': clean_xpathd_text(li.xpath('.//h3[@class="product-title"]/a/text()')),
            'volume': clean_xpathd_text(li.xpath('./zzzzzzzz')),
            'raw_price': '$' + clean_xpathd_text(li.xpath('(.//span[@class="woocommerce-Price-amount amount"]/text())[1]')),
...
def pdct_parsing(fpath, url, products): # TODO : modify xpaths
    ...
    products[url].update({
        'volume': clean_xpathd_text(tree.xpath('/zzzzzzz')),
        'pdct_img_main_url': clean_url(''.join(tree.xpath('//div[@id="prodimg"]/img/@src')[:1]), root_url),
        'ctg_denom_txt': ' '.join(tree.xpath('//div[@class="breadlinkwrap"]/text()')),
    })
    return products
...
###################
urls_ctgs_dict = {
    'whisky': 'https://sipwhiskey.com/product-category/whiskey,whisky/',
    'champagne': 'https://sipwhiskey.com/product-category/champagne/',
    'cognac': 'https://sipwhiskey.com/product-category/cognac/',
    #'sparkling': '',
    'vodka': 'https://sipwhiskey.com/product-category/vodka/',
    #'still_wines': '',
    'gin': 'https://sipwhiskey.com/product-category/gin/',
    'tequila': 'https://sipwhiskey.com/product-category/tequila/',
    #'red_wine': '',
    #'white_wine': '',
    'rum': 'https://sipwhiskey.com/product-category/rum/',
    'bourbon': 'https://sipwhiskey.com/product-category/bourbon/',
    'brandy': 'https://sipwhiskey.com/liquor-type/brandy/',
    'liquor': 'https://sipwhiskey.com/product-category/liqueur/',
}
...
kw_search_url = "https://sipwhiskey.com/?s={kw}&post_type=product&type_aws=true"  # TODO : modify URL
for kw in keywords:
    searches[kw] = []
