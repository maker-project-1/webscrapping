...
# # CTG page xpathing #
# ##################
ctg_page_test_url = 'https://www.hawesko.de/prosecco-champagner'
...
def ctg_parsing(fpath, ctg, categories, products):  # TODO : modify xpaths
    ...
    for li in tree.xpath('//div[@data-module="article"]'):
        ...
        products[produrl] = {
            'pdct_name_on_eretailer': clean_xpathd_text(li.xpath('.//span[@class="name text-uppercase"]//text()')),
            'volume': clean_xpathd_text(li.xpath('./zzzzzzz')),
            'raw_price': clean_xpathd_text(li.xpath('.//div[@class="price"]//span[@class="bulk-price"]/text()')),
            'raw_promo_price': clean_xpathd_text(li.xpath('.//div[@class="price-deprecated"]/span[@class="value"]//text()')),
...
# # KW page xpathing #
###################
search_page_test_url = 'https://www.hawesko.de/suche/?q=champagne'
...
def kw_parsing(fpath, kw, searches, products):  # TODO : modify xpaths
    ...
    for li in tree.xpath('//div[@data-module="article"]'):
        ...
        products[produrl] = {
            ...
            'raw_price': clean_xpathd_text(li.xpath('.//div[@class="price"]//span[@class="bulk-price"]/text()')[:0]),
            'raw_promo_price': clean_xpathd_text(li.xpath('.//div[@class="price-deprecated"]/span[@class="value"]//text()')),
            'pdct_img_main_url': "".join(li.xpath('./zzzz')),
        }
        products[produrl]['brnd'] = brm.find_brand(products[produrl]['pdct_name_on_eretailer'])['brand']
        print(products[produrl], produrl)
        products[produrl]['price'] = getprice(products[produrl]['raw_price'])
...
def pdct_parsing(fpath, url, products): # TODO : modify xpaths
    ...
    products[url].update({
        'volume': clean_xpathd_text(tree.xpath('//div[@class="characteristics accordion"]//div[@class="description-row" and contains(div, "Füllmenge")]/div[@class="description-value"]//text()')[:1], unicodedata_normalize=True),
        'pdct_img_main_url': clean_url(''.join(tree.xpath('//div[contains(@class, "article") and @data-fragment]//picture//img/@src')[:1]), root_url),
        'ctg_denom_txt': ' '.join(tree.xpath('//div[@class="breadcrumb"]//text()')),
    })
    return products
...
kw_search_url = "https://www.hawesko.de/suche/?q={kw}"  # TODO : modify URL
for kw in keywords:
    searches[kw] = []
