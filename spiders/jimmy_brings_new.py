...
def ctg_parsing(fpath, ctg, categories, products):  # TODO : modify xpaths
    ...
    for li in tree.xpath('//li[@class="menu-item"]'):
        if not li.xpath('.//h4//text()'):
            continue
        produrl = li.xpath('.//h4//text()')[0]
        produrl = parse_qs(urlsplit(produrl).query)['url'][0] if 'url' in parse_qs(urlsplit(produrl).query) else produrl
        products[produrl] = {
            'pdct_name_on_eretailer': clean_xpathd_text(li.xpath('.//h4//text()')),
            'volume': clean_xpathd_text(li.xpath('./zzzz')),
            'raw_price': clean_xpathd_text(li.xpath('.//div[@class="moduleResult"]/div[@class="mpricenew"]//text()')),
            'raw_promo_price': clean_xpathd_text(li.xpath('.//zzzzzzzzzz')),
            'pdct_img_main_url': "".join(li.xpath('.//div[@class="menuimg"]/img/@src')),
        }
...
def pdct_parsing(fpath, url, products): # TODO : modify xpaths
    ...
    products[url].update({
        ...
        'pdct_img_main_url': clean_url(''.join(tree.xpath('/zzzzz')), root_url),
        'ctg_denom_txt': ' '.join(tree.xpath('/zzzzzzzzzzz')),
    })
    return products
