...
# # CTG page xpathing #
# ##################
ctg_page_test_url = 'https://www.klwines.com/Wines'
...
def ctg_parsing(fpath, ctg, categories, products):  # TODO : modify xpaths
    ...
    for li in tree.xpath('//div[contains(@class, "results-block")]/div[contains(@class, "result")]'):
        ...
        products[produrl] = {
            'pdct_name_on_eretailer': clean_xpathd_text(li.xpath('.//div[contains(@class, "result-desc")]/a//text()')),
            'volume': clean_xpathd_text(li.xpath('./zzzzzzzzzzzzzz')),
            'raw_price': clean_xpathd_text(li.xpath('.//span/span/strong//text()')),
            'raw_promo_price': clean_xpathd_text(li.xpath('.//span/span/fds//text()')),
...
# # KW page xpathing #
###################
search_page_test_url = 'https://www.klwines.com/Champagne'
...
def kw_parsing(fpath, kw, searches, products):  # TODO : modify xpaths
    ...
    for li in tree.xpath('//div[contains(@class, "results-block")]/div[contains(@class, "result")]'):
        ...
        products[produrl] = {
            ...
            'raw_price': clean_xpathd_text(li.xpath('.//span/span/strong//text()')),
            'raw_promo_price': clean_xpathd_text(li.xpath('.//span/span/fds//text()')),
            'pdct_img_main_url': "".join(li.xpath('./zzzzzzz')),
        }
        products[produrl]['brnd'] = brm.find_brand(products[produrl]['pdct_name_on_eretailer'])['brand']
        print(products[produrl], produrl)
        products[produrl]['price'] = getprice(products[produrl]['raw_price'])
...
def pdct_parsing(fpath, url, products): # TODO : modify xpaths
    ...
    products[url].update({
        'volume': clean_xpathd_text(tree.xpath('//div[@class="result-desc"]//h1//text()')),
        'pdct_img_main_url': clean_url(''.join(tree.xpath('//div[@class="productImg"]//img/@src')), root_url).replace('/images/btn_zoom.gif',''),
...
kw_search_url = "http://www.klwines.com/Products/r?s=System.Web.HttpCookie&d=0&t={kw}&z=False&o=-1&displayCount=100"  # TODO : modify URL
for kw in keywords:
    searches[kw] = []
