...
# # CTG page xpathing #
# ##################
ctg_page_test_url = 'https://www.samsclub.com/sams/alcohol/1040209.cp?clubId=6376&limit=48&offset=96&searchCategoryId=1040209&selectedFilter=all&sortKey=p_sales_rank&sortOrder=1&viewMode=grid'
...
def ctg_parsing(fpath, ctg, categories, products):  # TODO : modify xpaths
    ...
    for li in tree.xpath('//ul[@role="list"]/li'):
...
# # KW page xpathing #
###################
search_page_test_url = 'https://www.samsclub.com/sams/search/searchResults.jsp?searchTerm=champagne&searchCategoryId=all&xid=hdr_search-typeahead_champagne'
...
urls_ctgs_dict = {
    'total': 'https://www.samsclub.com/sams/alcohol/1040209.cp?clubId=6376&limit={page}&offset=96&searchCategoryId=1040209&selectedFilter=all&sortKey=p_sales_rank&sortOrder=1&viewMode=grid'
}
...
for ctg, url in urls_ctgs_dict.items():
    ...
    for p in range(100):
        ...
        if not op.exists(fpath):
            driver.get(url.format(page=p+48))
            sleep(2)
...
kw_search_url = "https://www.samsclub.com/sams/search/searchResults.jsp?searchTerm={kw}&searchCategoryId=all&xid=hdr_search-typeahead_{kw}"  # TODO : modify URL
for kw in keywords:
    searches[kw] = []
    number_of_pdcts_in_kw_search = 0
    if not op.exists(fpath_namer(shop_id, 'search', kw, 0)):
        driver.get(kw_search_url.format(kw=kw, page=48))
