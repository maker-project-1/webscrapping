...
# # CTG page xpathing #
# ##################
ctg_page_test_url = 'https://groceries.asda.com/dept/drinks/spirits-pre-mixed-drinks/_/102571'
...
def ctg_parsing(fpath, ctg, categories, products):  # TODO : modify xpaths
    ...
    for li in tree.xpath('//div[@data-cid]'):
        if not li.xpath('.//span[@class="title productTitle"]/a/@href'):
            continue
        produrl = li.xpath('.//span[@class="title productTitle"]/a/@href')[0]
        produrl = parse_qs(urlsplit(produrl).query)['url'][0] if 'url' in parse_qs(urlsplit(produrl).query) else produrl
        products[produrl] = {
            'pdct_name_on_eretailer': clean_xpathd_text(li.xpath('.//span[@class="title productTitle"]//span//text()')),
            'volume': clean_xpathd_text(li.xpath('.//div[@class="shop-product-size"]//text()')),
            'raw_price': clean_xpathd_text(li.xpath('.//span[@class="price"]/span[last()]//text()|.//span[@class="wasprice"]/span[2]//text()')[-1:]),
...
# # KW page xpathing #
###################
search_page_test_url = 'https://groceries.asda.com/search/{kw}'
...
def kw_parsing(fpath, kw, searches, products):  # TODO : modify xpaths
    ...
    for li in tree.xpath('//div[@data-cid]'):
        ...
        products[produrl] = {
            'pdct_name_on_eretailer': clean_xpathd_text(li.xpath('.//span[@class="title productTitle"]//span//text()')),
            'volume': clean_xpathd_text(li.xpath('.//div[@class="shop-product-size"]//text()')),
            'raw_price': clean_xpathd_text(li.xpath('.//span[@class="price"]/span[last()]//text()')),
            'raw_promo_price': clean_xpathd_text(li.xpath('.//span[contains(@id, "old-price-")]/@data-price-amount')),
            'pdct_img_main_url': "".join(li.xpath('.//img/@src')[:1]),
        }
        products[produrl]['brnd'] = brm.find_brand(products[produrl]['pdct_name_on_eretailer'])['brand']
...
kw_search_url = "https://groceries.asda.com/search/{kw}"  # TODO : modify URL
for kw in keywords:
    searches[kw] = []
