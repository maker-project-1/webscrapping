...
# # CTG page xpathing #
# ##################
ctg_page_test_url = 'https://www.astorwines.com/WineSearchResult.aspx?p=1&search=Advanced&searchtype=Contains&term=&cat=1&color=Red'
...
def ctg_parsing(fpath, ctg, categories, products):  # TODO : modify xpaths
    ...
    for li in tree.xpath('//*[@id="search-results"]//div[@class="item-teaser"]'):
        if not li.xpath('.//h2[contains(@class, "item-name")]/a/@href'):
            continue
        produrl = li.xpath('.//h2[contains(@class, "item-name")]/a/@href')[0]
        produrl = parse_qs(urlsplit(produrl).query)['url'][0] if 'url' in parse_qs(urlsplit(produrl).query) else produrl
        products[produrl] = {
            'pdct_name_on_eretailer': clean_xpathd_text(li.xpath('.//h2[contains(@class, "item-name")]/a/text()')),
            'volume': clean_xpathd_text(li.xpath('.//span[@class="small"]//text()')),
            'raw_price': clean_xpathd_text(li.xpath('.//span[@class="price"]//text()')),
            'raw_promo_price': clean_xpathd_text(li.xpath('.//div[contains(@class, "bottle-price-container")]/span[last()-1]/text()|.//span[@class="price-value price-old price-bottle"]//text()')),
            'pdct_img_main_url': "".join(li.xpath('.//img[contains(@id, "imgItem")]/@src')[:1]),
        }
...
# # KW page xpathing #
###################
search_page_test_url = 'http://www.astorwines.com/WineSearchResult.aspx?p=1&search=Advanced&searchtype=Contains&tm={kw}&Page={page}'
...
def kw_parsing(fpath, kw, searches, products):  # TODO : modify xpaths
    ...
    for li in tree.xpath('//*[@id="search-results"]//div[@class="item-teaser"]'):
        ...
        products[produrl] = {
            'pdct_name_on_eretailer': clean_xpathd_text(li.xpath('.//h2[contains(@class, "item-name")]/a/text()')),
            'volume': clean_xpathd_text(li.xpath('.//span[@class="small"]//text()')),
            'raw_price': clean_xpathd_text(li.xpath('.//div[contains(@class, "bottle-price-container")]/span[last()-1]/text()|.//span[@class="price-value price-old price-bottle"]//text()')),
            'raw_promo_price': clean_xpathd_text(li.xpath('.//div[contains(@class, "bottle-price-container")]//span[@class="price-sale"]/text()')),
            'pdct_img_main_url': "".join(li.xpath('.//img[contains(@id, "imgItem")]/@src')[:1]),
        }
        products[produrl]['brnd'] = brm.find_brand(products[produrl]['pdct_name_on_eretailer'])['brand']
        print(products[produrl], produrl)
...
def pdct_parsing(fpath, url, products): # TODO : modify xpaths
    ...
    products[url].update({
        ...
        'pdct_img_main_url': clean_url(''.join(tree.xpath('.//img[contains(@id, "imgItem")]/@src')[:1]), root_url),
        'ctg_denom_txt': ' '.join(tree.xpath('xpath')),
    })
    return products
...
kw_search_url = "http://www.astorwines.com/WineSearchResult.aspx?p=1&search=Advanced&searchtype=Contains&tm={kw}&Page=1"  # TODO : modify URL
for kw in keywords:
    searches[kw] = []
