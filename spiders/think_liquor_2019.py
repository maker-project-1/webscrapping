...
shop_id = 'think_liquor'
root_url = 'https://thinkliquor.com/'
...
def ctg_parsing(fpath, ctg, categories, products):  # TODO : modify xpaths
    ...
    for li in tree.xpath('//div[@class="products-list-wrapper"]/div/div'):
        if not li.xpath('.//a/@href'):
            continue
        produrl = li.xpath('.//a/@href')[0]
        produrl = parse_qs(urlsplit(produrl).query)['url'][0] if 'url' in parse_qs(urlsplit(produrl).query) else produrl
        products[produrl] = {
            'pdct_name_on_eretailer': clean_xpathd_text(li.xpath('.//div[@class="product-item-name"]//text()')),
            'volume': clean_xpathd_text(li.xpath('./zzzzzzzzzzzz')),
            'raw_price': clean_xpathd_text(li.xpath('.//span[@class="regular-price"]//text()')).replace(" ",""),
            'raw_promo_price': clean_xpathd_text(li.xpath('./zzzzzzzzzzzz')),
            'pdct_img_main_url': "".join(li.xpath('.//img[@class="product-img"]/@src')[:1]),
        }
...
# # KW page xpathing #
###################
search_page_test_url = 'https://thinkliquor.com/catalogsearch/result/?q=red+wine'
...
def kw_parsing(fpath, kw, searches, products):  # TODO : modify xpaths
    ...
    for li in tree.xpath('//div[@class="products-list-wrapper"]/div/div'):
        ...
        products[produrl] = {
            'pdct_name_on_eretailer': clean_xpathd_text(li.xpath('.//div[@class="product-item-name"]//text()')),
            'volume': clean_xpathd_text(li.xpath('./zzzzzzzzzz')),
            'raw_price': clean_xpathd_text(li.xpath('.//span[@class="regular-price"]//text()')).replace(" ",""),
            'raw_promo_price': clean_xpathd_text(li.xpath('./zzzzzzzzzz')),
            'pdct_img_main_url': "".join(li.xpath('.//img[@class="product-img"]/@src')[0]),
        }
        products[produrl]['brnd'] = brm.find_brand(products[produrl]['pdct_name_on_eretailer'])['brand']
        print(products[produrl], produrl)
        products[produrl]['price'] = getprice(products[produrl]['raw_price'])
...
def pdct_parsing(fpath, url, products): # TODO : modify xpaths
    ...
    products[url].update({
        'volume': clean_xpathd_text(tree.xpath('//div[@class="product-filterable-attributes"]//text()')),
        'pdct_img_main_url': clean_url(''.join(tree.xpath('//img[@class="gallery-image visible"]//@src')[:1]), root_url),
        'ctg_denom_txt': ' '.join(tree.xpath('//ul[@class="breadcrumb"]//text()')[:100]),
    })
    return products
...
urls_ctgs_dict = {
    'whisky': 'https://thinkliquor.com/usa-american-whiskey',
    'champagne': 'https://thinkliquor.com/sparkling-wine',
    'cognac': 'https://thinkliquor.com/cognac-brandy',
    'sparkling': 'https://thinkliquor.com/sparkling-wine',
    'vodka': 'https://thinkliquor.com/vodka',
    'still_wines': 'https://thinkliquor.com/specialty-wine',
    'gin': 'https://thinkliquor.com/gin',
    'tequila': 'https://thinkliquor.com/tequila',
    'red_wine': 'https://thinkliquor.com/red-wine',
    'white_wine': 'https://thinkliquor.com/gin',
    'rum': 'https://thinkliquor.com/rum',
    'bourbon': 'https://thinkliquor.com/us-bourbon-whiskey',
    'brandy': 'https://thinkliquor.com/brandy-marc',
    'liquor': 'https://thinkliquor.com/all-liqueurs',
}
...
kw_search_url = "https://thinkliquor.com/catalogsearch/result/?q={kw}"  # TODO : modify URL
for kw in keywords:
    searches[kw] = []
