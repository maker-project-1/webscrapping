...
# # CTG page xpathing #
# ##################
ctg_page_test_url = 'https://www.vintagecellars.com.au/Red%20Wine?facets=origincountry%3dFrance'
...
def ctg_parsing(fpath, ctg, categories, products):  # TODO : modify xpaths
    ...
    for li in tree.xpath('//ul[@class="productList"]/li'):
        if not li.xpath('.//a[contains(@id, "ProductThumbnailLink")]/@href'):
            continue
        produrl = li.xpath('.//a[contains(@id, "ProductThumbnailLink")]/@href')[0]
        ...
        products[produrl] = {
            ...
            'raw_promo_price': clean_xpathd_text(li.xpath('.//span[contains(@class, "price sqd")]')),
            'pdct_img_main_url': "".join(li.xpath('.//a[contains(@id, "ProductThumbnailLink")]//img/@src')[:1]),
        }
...
# # KW page xpathing #
###################
search_page_test_url = 'https://www.vintagecellars.com.au/Search?q=champagne'
...
def pdct_parsing(fpath, url, products): # TODO : modify xpaths
    ...
    products[url].update({
        ...
        'ctg_denom_txt': ' '.join(tree.xpath('//ul[@class="breadcrumbs"]/li/a/text()')),
    })
    return products
...
kw_search_url = "https://www.vintagecellars.com.au/Search?q={kw}&show=100&page={page}"  # TODO : modify URL
for kw in keywords:
    searches[kw] = []
