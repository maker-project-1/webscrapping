...
# # CTG page xpathing #
# ##################
ctg_page_test_url = 'https://www.isetan.com.sg/liquor/red-wine/'
...
def ctg_parsing(fpath, ctg, categories, products):  # TODO : modify xpaths
    ...
    for li in tree.xpath('//div[@class="content"]/ul/li[@class="col"]'):
        ...
        products[produrl] = {
            'pdct_name_on_eretailer': clean_xpathd_text(li.xpath('.//p[@class="text"]//a//text()')[0]),
            'volume': clean_xpathd_text(li.xpath('.//p[@class="text"]/a/font//text()')),
            'raw_price': clean_xpathd_text(li.xpath('.//p[@class="text"]/a//span//text()')),
            'raw_promo_price': clean_xpathd_text(li.xpath('.//xpath/text()')),
            'pdct_img_main_url': "".join(li.xpath('.//p[@class="image"]//img/@src')[0]),
        }
...
# # KW page xpathing #
###################
search_page_test_url = 'https://www.isetan.com.sg/search.php?search_query=champagne&section=product'
...
def kw_parsing(fpath, kw, searches, products):  # TODO : modify xpaths
    ...
    for li in tree.xpath('//div[@class="content"]/ul/li[@class="col"]'):
        ...
        produrl = parse_qs(urlsplit(produrl).query)['url'][0] if 'url' in parse_qs(urlsplit(produrl).query) else produrl
        products[produrl] = {
            'pdct_name_on_eretailer': clean_xpathd_text(li.xpath('.//p[@class="text"]//a//text()')),
            'volume': clean_xpathd_text(li.xpath('.//p[@class="text"]/a/font//text()')),
            'raw_price': clean_xpathd_text(li.xpath('.//p[@class="text"]/a//span//text()')),
            'raw_promo_price': clean_xpathd_text(li.xpath('.//xpath/text()')),
            'pdct_img_main_url': "".join(li.xpath('.//p[@class="image"]//img/@src')[:1]),
        }
        products[produrl]['brnd'] = brm.find_brand(products[produrl]['pdct_name_on_eretailer'])['brand']
        print(products[produrl], produrl)
        products[produrl]['price'] = getprice(products[produrl]['raw_price'])
