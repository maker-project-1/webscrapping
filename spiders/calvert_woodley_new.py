...
# # CTG page xpathing #
# ##################
ctg_page_test_url = 'https://www.calvertwoodley.com/wines/'
...
def ctg_parsing(fpath, ctg, categories, products):  # TODO : modify xpaths
    ...
    for li in tree.xpath('//div[contains(@class, "product-list")]/div'):
        if not li.xpath('.//table[contains(@class, "wf_content")]//a/@href'):
            continue
        produrl = li.xpath('.//table[contains(@class, "wf_content")]//a/@href')[0]
        produrl = parse_qs(urlsplit(produrl).query)['url'][0] if 'url' in parse_qs(urlsplit(produrl).query) else produrl
        products[produrl] = {
            'pdct_name_on_eretailer': clean_xpathd_text(li.xpath('.//a[contains(@class, "rebl15")]//text()')),
            'volume': clean_xpathd_text(li.xpath('.//div[@class="shop-product-size"]//text()')),
            'raw_price': clean_xpathd_text(li.xpath('.//td[@class="rpwrap"]/table[2]//tr[1]/td[1]//text()')),
            'raw_promo_price': clean_xpathd_text(li.xpath('.//td[@class="rpwrap"]/table[3]//tr[1]/td[1]//text()')),
...
# # KW page xpathing #
###################
search_page_test_url = 'https://www.calvertwoodley.com/websearch_results.html?kw=champagne'
...
def pdct_parsing(fpath, url, products): # TODO : modify xpaths
    ...
    products[url].update({
        'volume': clean_xpathd_text(tree.xpath('//*[@id="content"]/div/span/h1//text()')[:3], unicodedata_normalize=True),
        'pdct_img_main_url': clean_url(''.join(tree.xpath('//div[contains(@class, "gallery_sec")]//img/@src')[:1]), root_url),
        'ctg_denom_txt': ' '.join(tree.xpath('//*[@id="prodata"]//text()')),
    })
    return products
...
kw_search_url = "https://www.calvertwoodley.com/websearch_results.html?page={page}&kw={kw}&sortby=sort_item_order&l=100"  # TODO : modify URL
for kw in keywords:
    searches[kw] = []
