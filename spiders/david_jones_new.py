...
# # CTG page xpathing #
# ##################
ctg_page_test_url = 'https://www.davidjones.com/home-and-food/food-and-wine/wine-champagne-and-spirits'
...
def ctg_parsing(fpath, ctg, categories, products):  # TODO : modify xpaths
    ...
    for li in tree.xpath('//div[@class="item isUpdated"]'):
        ...
        products[produrl] = {
            'pdct_name_on_eretailer': clean_xpathd_text(li.xpath('.//h4//text()')),
            'volume': clean_xpathd_text(li.xpath('./h4/a/text()')),
            'raw_price': clean_xpathd_text(li.xpath('.//*[@class="price was"]//text()')),
            'raw_promo_price': clean_xpathd_text(li.xpath('.//*[@itemprop="price"]//text()')),
            'pdct_img_main_url': "".join(li.xpath('.//figure/a/img/@src')),
        }
...
# # KW page xpathing #
###################
search_page_test_url = 'https://www.davidjones.com/home-and-food/food-and-wine/wine-champagne-and-spirits'
...
def pdct_parsing(fpath, url, products): # TODO : modify xpaths
    ...
    products[url].update({
        'volume': clean_xpathd_text(tree.xpath('//div[@class="content long-description"]//text()'), unicodedata_normalize=True),
        'pdct_img_main_url': clean_url(''.join(tree.xpath('//section[@class="product-detail"]//img[@itemprop="image"]/@src')), root_url),
        'ctg_denom_txt': ' '.join(tree.xpath('//nav[@id="breadcrumbs"]//text()')),
    })
    return products
...
kw_search_url = "https://search.www.davidjones.com/search?w={kw}"  # TODO : modify URL
for kw in keywords:
    searches[kw] = []
