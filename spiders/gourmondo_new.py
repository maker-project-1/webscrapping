...
# # CTG page xpathing #
# ##################
ctg_page_test_url = 'https://www.gourmondo.de/champagner-schaumwein/champagner?etcc_cu=onsite&etcc_cmp_onsite=1836_champagner&etcc_med_onsite=1836_topnav_champagner#pageNumber=1&sortOption=topseller&pageSize=100'
...
def ctg_parsing(fpath, ctg, categories, products):  # TODO : modify xpaths
    ...
    for li in tree.xpath('//div[@class="product"]'):
        ...
        products[produrl] = {
            ...
            'raw_promo_price': clean_xpathd_text(li.xpath('.//div[@class="price"]//span[@class="initial-price reduced"]//text()')),
            'pdct_img_main_url': "".join(li.xpath('.//img[@itemprop="contentUrl"]/@src')[:1]),
        }
        if len(products[produrl]['raw_promo_price']) > 0:
...
# # KW page xpathing #
###################
search_page_test_url = 'https://www.gourmondo.de/search/?text=champagne#pageNumber=1&sortOption=topseller&pageSize=100'
