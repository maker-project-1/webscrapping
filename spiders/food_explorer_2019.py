...
shop_id = 'food_explorer'
root_url = 'https://www.foodexplorer.com/de'
...
# # KW page xpathing #
###################
search_page_test_url = 'https://www.foodexplorer.com/de/search?text=Champagner'
...
def kw_parsing(fpath, kw, searches, products):  # TODO : modify xpaths
    ...
    for li in tree.xpath('//div[@class="o-grid o-grid--wide"]/a'):
        if not li.xpath('.//@href'):
            continue
        produrl = li.xpath('//@href')[0]
        produrl = parse_qs(urlsplit(produrl).query)['url'][0] if 'url' in parse_qs(urlsplit(produrl).query) else produrl
        products[produrl] = {
            'pdct_name_on_eretailer': clean_xpathd_text(li.xpath('.//h3[@class="c-h3"]//text()')),
            'volume': clean_xpathd_text(li.xpath('./zzzzzzz')),
            'raw_price': clean_xpathd_text(li.xpath('.//span[@class="price-new"]//text()')),
...
def pdct_parsing(fpath, url, products): # TODO : modify xpaths
    ...
    products[url].update({
        ...
        'pdct_img_main_url': clean_url(''.join(tree.xpath('//a[@data-lightbox="product-gallery"]//img/@src')[0]), root_url),
        'ctg_denom_txt': ' '.join(tree.xpath('//ul[@class="c-breadcrumb o-list u-hidden u-flex@sm"]//text()')),
    })
    return products
...
kw_search_url = "https://www.foodexplorer.com/de/search?text={kw}&page={page}"  # TODO : modify URL
for kw in keywords:
    searches[kw] = []
