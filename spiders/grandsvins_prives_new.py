...
# # CTG page xpathing #
# ##################
ctg_page_test_url = 'https://grandsvins-prives.com/10-les-bulles#'
...
def ctg_parsing(fpath, ctg, categories, products):  # TODO : modify xpaths
    ...
    for li in tree.xpath('//ul[contains(@class,"product_list")]/li'):
        if not li.xpath('.//h5/a/@href'):
            continue
        produrl = li.xpath('.//h5/a/@href')[0]
        produrl = parse_qs(urlsplit(produrl).query)['url'][0] if 'url' in parse_qs(urlsplit(produrl).query) else produrl
        products[produrl] = {
            'pdct_name_on_eretailer': clean_xpathd_text(li.xpath('.//h5//text()')),
            'volume': clean_xpathd_text(li.xpath('./zzzzzz')),
            'raw_price': clean_xpathd_text(li.xpath('.//div/span[@class="actual-price"]//text()')),
            'raw_promo_price': clean_xpathd_text(li.xpath('.//div/span[@class="old-price"]//text()')),
...
# # KW page xpathing #
###################
search_page_test_url = 'https://grandsvins-prives.com/recherche?controller=search&orderby=position&orderway=desc&search_query=champagne&submit_search='
...
def pdct_parsing(fpath, url, products): # TODO : modify xpaths
    ...
    products[url].update({
        'volume': clean_xpathd_text(tree.xpath('//div[@class="product-quantity"]//text()')),
        'pdct_img_main_url': clean_url(''.join(tree.xpath('//img[@id="bigpic"]/@src')), root_url),
...
urls_ctgs_dict = {
    'champagne': 'https://grandsvins-prives.com/10-les-bulles#region=0&aoc=CHAMPAGNE&producteur=0&prix=0&couleur=0',
    'sparkling': 'https://grandsvins-prives.com/10-les-bulles#region=0&aoc=CRÉMANT-DE-BORDEAUX&producteur=0&prix=0&couleur=0',
    'still_wines': 'https://grandsvins-prives.com/18-nos-blancs/',
    'red_wine': 'https://grandsvins-prives.com/20-bordeaux-grands-crus',
    'whisky': 'https://grandsvins-prives.com/recherche?controller=search&orderby=position&orderway=desc&search_query=whisky&submit_search=',
    'cognac':'https://grandsvins-prives.com/83-spiritueux#region=0&aoc=COGNAC&producteur=0&prix=0&couleur=0',
    'vodka': 'https://grandsvins-prives.com/83-spiritueux#region=0&aoc=POLOGNE&producteur=0&prix=0&couleur=0',
    'gin': 'https://grandsvins-prives.com/83-spiritueux#appellation-gin',
    'rum': 'https://grandsvins-prives.com/83-spiritueux#/appellation-rhum_traditionnel-rhum_vieux_traditionnel',
}
...
kw_search_url = "https://grandsvins-prives.com/recherche?controller=search&orderby=position&orderway=desc&search_query={kw}&submit_search="  # TODO : modify URL
for kw in keywords:
    searches[kw] = []
