...
# # CTG page xpathing #
# ##################
ctg_page_test_url = 'https://applejack.com/Wine'
...
def ctg_parsing(fpath, ctg, categories, products):  # TODO : modify xpaths
    ...
    for li in tree.xpath('//div[@itemprop="itemListElement"]'):
        ...
        products[produrl] = {
            'pdct_name_on_eretailer': clean_xpathd_text(li.xpath('.//span[@itemprop="name"]//text()')),
            'volume': clean_xpathd_text(li.xpath('.//div[@class="shop-product-size"]//text()')),
            'raw_price': clean_xpathd_text(li.xpath('.//span[contains(@class, "price-lead")]//text()')).split('to ')[-1],
...
# # KW page xpathing #
###################
search_page_test_url = 'https://applejack.com/custitem_item_type/Item?keywords=champagne&order=relevance:asc'
...
def pdct_parsing(fpath, url, products): # TODO : modify xpaths
    ...
    products[url].update({
        'volume': clean_xpathd_text(tree.xpath('//div[@class="item-description"]//text()')[:3], unicodedata_normalize=True),
        'pdct_img_main_url': clean_url(''.join(tree.xpath('//div[@class="item-details-image-gallery"]//img/@src')[:1]), root_url),
...
kw_search_url = "https://applejack.com/custitem_item_type/Item?keywords={kw}"  # TODO : modify URL
for kw in keywords:
    searches[kw] = []
