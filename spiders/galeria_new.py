...
from ers import all_keywords_de as keywords, fpath_namer, mh_brands, clean_url, shop_inventory_lw_csv
...
# # CTG page xpathing #
# ##################
ctg_page_test_url = 'https://www.galeria-kaufhof.de/wein-gourmet/spirituosen/rum/'
...
def ctg_parsing(fpath, ctg, categories, products):  # TODO : modify xpaths
    ...
    for li in tree.xpath('//div[@class="gk-article gk-article--big"]'):
        if not li.xpath('.//a[@class="gk-article__link"]/@href'):
            continue
        produrl = li.xpath('.//a[@class="gk-article__link"]/@href')[0]
        produrl = parse_qs(urlsplit(produrl).query)['url'][0] if 'url' in parse_qs(urlsplit(produrl).query) else produrl
        products[produrl] = {
            'pdct_name_on_eretailer': clean_xpathd_text(li.xpath('.//span[@class="gk-article__description"]//text()')),
            'volume': clean_xpathd_text(li.xpath('.//span[@class="gk-article__description"]//text()')),
            'raw_price': (clean_xpathd_text(li.xpath('.//div[@class="gk-price"]/text()')) or clean_xpathd_text(li.xpath('.//ins[@class="gk-price-new"]/text()'))),
            'raw_promo_price': clean_xpathd_text(li.xpath('.//span[contains(@id, "old-price-")]/@data-price-amount')),
            'pdct_img_main_url': ("".join(li.xpath('.//img[@itemprop="image"]/@data-src')) or "".join(li.xpath('.//img[@itemprop="image"]/@src'))),
        }
        products[produrl]['brnd'] = brm.find_brand(products[produrl]['pdct_name_on_eretailer'])['brand']
        print(products[produrl], produrl)
        products[produrl]['price'] = getprice(products[produrl]['raw_price'])
        products[produrl]['promo_price'] = getprice(products[produrl]['raw_promo_price'])
        products[produrl]['pdct_img_main_url'] = clean_url(products[produrl]['pdct_img_main_url'][0], root_url)
...
# # KW page xpathing #
###################
search_page_test_url = 'https://www.galeria-kaufhof.de/search?q=champagne'
...
def pdct_parsing(fpath, url, products): # TODO : modify xpaths
    ...
    products[url].update({
        ...
        'pdct_img_main_url': clean_url(''.join(tree.xpath('//section[@class="ev-product__images ev-images"]//img/@src')[0]), root_url),
        'ctg_denom_txt': ' '.join(tree.xpath('//div[@class="gk-breadcrumb"]//text()')),
    })
    return products
...
kw_search_url = "https://www.galeria-kaufhof.de/search?q={kw}&page={page}"  # TODO : modify URL
for kw in keywords:
    searches[kw] = []
