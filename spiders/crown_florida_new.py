...
# # CTG page xpathing #
# ##################
ctg_page_test_url = 'https://www.crownwineandspirits.com/gin/'
...
def ctg_parsing(fpath, ctg, categories, products):  # TODO : modify xpaths
    ...
    for li in tree.xpath('//ul[contains(@class, "ProductList")]/li'):
        if not li.xpath('.//div[contains(@class, "ProductDetails")]//a/@href'):
            continue
        produrl = li.xpath('.//div[contains(@class, "ProductDetails")]//a/@href')[0]
        produrl = parse_qs(urlsplit(produrl).query)['url'][0] if 'url' in parse_qs(urlsplit(produrl).query) else produrl
        products[produrl] = {
            'pdct_name_on_eretailer': clean_xpathd_text(li.xpath('.//div[contains(@class, "ProductDetails")]//a//text()')),
            'volume': clean_xpathd_text(li.xpath('./zzzzzzzzzzzz')),
            'raw_price': clean_xpathd_text(li.xpath('.//em/text()')),
            'raw_promo_price': clean_xpathd_text(li.xpath('.//em//strike//text()')),
            'pdct_img_main_url': "".join(li.xpath('./zzzzzzz')[:1]),
        }
...
# # KW page xpathing #
###################
search_page_test_url = 'https://www.crownwineandspirits.com/search?search_query=champagne'
...
def kw_parsing(fpath, kw, searches, products):  # TODO : modify xpaths
    ...
    for li in tree.xpath('//ul[contains(@class, "ProductList")]/li'):
        ...
        products[produrl] = {
            ...
            'raw_price': clean_xpathd_text(li.xpath('.//em/span/text()')),#[:2]
            'raw_promo_price': clean_xpathd_text(li.xpath('.//em//strike//text()')),
...
kw_search_url = "https://www.crownwineandspirits.com/search?search_query={kw}#page:{page}"  # TODO : modify URL
for kw in keywords:
    searches[kw] = []
