...
shop_id = 'grand_vin_wine_merchants'
root_url = 'https://grandvinwinemerchants.com/'
...
# # CTG page xpathing #
# ##################
ctg_page_test_url = 'https://grandvinwinemerchants.com/champagne/champagne/'
...
def ctg_parsing(fpath, ctg, categories, products):  # TODO : modify xpaths
    ...
    for li in tree.xpath('//div[@class="SearchContainer"]/ul/li'):
        if not li.xpath('.//a/@href'):
            continue
        produrl = li.xpath('.//a/@href')[0]
        produrl = parse_qs(urlsplit(produrl).query)['url'][0] if 'url' in parse_qs(urlsplit(produrl).query) else produrl
        products[produrl] = {
            'pdct_name_on_eretailer': clean_xpathd_text(li.xpath('.//div[@class="ProductDetails"]/strong//text()')),
            'volume': clean_xpathd_text(li.xpath('.//div[@class="ProductDetails"]/strong//text()')),
            'raw_price': clean_xpathd_text(li.xpath('(.//div[@class="ProductPriceRating"]/strong/*[not(self::strike)]/text() | .//div[@class="ProductPriceRating"]/strong/text())[1]')),
            'raw_promo_price': clean_xpathd_text(li.xpath('.//strike/text()')),
            'pdct_img_main_url': "".join(li.xpath('.//div[@class="ProductImage QuickView"]//img/@src')[0]),
        }
...
# # KW page xpathing #
###################
search_page_test_url = 'https://grandvinwinemerchants.com/search.php?search_query=champagne&Search='
...
def kw_parsing(fpath, kw, searches, products):  # TODO : modify xpaths
    ...
    for li in tree.xpath('//div[@class="SearchContainer"]/ul/li'):
        ...
        products[produrl] = {
            ...
            'raw_price': clean_xpathd_text(li.xpath('(.//div[@class="ProductPriceRating"]/strong/*[not(self::strike)]/text() | .//div[@class="ProductPriceRating"]/strong/text())[1]')),
            'raw_promo_price': clean_xpathd_text(li.xpath('.//strike/text()')),
            'pdct_img_main_url': "".join(li.xpath('.//div[@class="ProductImage QuickView"]//img/@src')[:1]),
        }
        products[produrl]['brnd'] = brm.find_brand(products[produrl]['pdct_name_on_eretailer'])['brand']
        print(products[produrl], produrl)
        products[produrl]['price'] = getprice(products[produrl]['raw_price'])
...
def pdct_parsing(fpath, url, products): # TODO : modify xpaths
    ...
    products[url].update({
        'volume': clean_xpathd_text(tree.xpath('//h1//text()')),
        'pdct_img_main_url': clean_url(''.join(tree.xpath('//div[@class="ProductThumbImage"]//img/@src')[:1]), root_url),
...
urls_ctgs_dict = {
#    'whisky': '',
    'champagne': 'https://grandvinwinemerchants.com/champagne/champagne/',
#    'cognac': '',
    'sparkling': 'https://grandvinwinemerchants.com/champagne/sparkling/',
#    'vodka': '',
#    'still_wines': '',
#    'gin': '',
#    'tequila': '',
...
kw_search_url = "https://grandvinwinemerchants.com/search.php?search_query={kw}&Search="  # TODO : modify URL
for kw in keywords:
    searches[kw] = []
