...
from ers import all_keywords_fr as keywords, fpath_namer, mh_brands, clean_url, shop_inventory_lw_csv
...
# # CTG page xpathing #
# ##################
ctg_page_test_url = 'https://fd10-courses.leclercdrive.fr/magasin-159201-Colombes-Michelet/rayon-284518-Champagnes-Mousseux-Cidres.aspx?Filtres=4-284542'
...
def ctg_parsing(fpath, ctg, categories, products):  # TODO : modify xpaths
    ...
    for li in tree.xpath('//ul[@id="ulListeProduits"]/li[not(contains(@class, "navailable"))]'):
        if not li.xpath('.//a[contains(@class, "_Product")]//text()'):
            continue
        produrl = li.xpath('.//a[contains(@class, "_Product")]//text()')[0]
        produrl = parse_qs(urlsplit(produrl).query)['url'][0] if 'url' in parse_qs(urlsplit(produrl).query) else produrl
        products[produrl] = {
            'pdct_name_on_eretailer': clean_xpathd_text(li.xpath('.//a[contains(@class, "_Product")]//text()')),
            'volume': clean_xpathd_text(li.xpath('.//a[contains(@class, "_Product")]//text()')),
            'raw_price': clean_xpathd_text(li.xpath('.//p[contains(@class, "_PrixUnitaire")]//text()')),
            'raw_promo_price': clean_xpathd_text(li.xpath('.//sdsf//text()')),
            'pdct_img_main_url': "".join(li.xpath('.//a[contains(@class, "_Product")]/img/@src')),
        }
...
# # KW page xpathing #
###################
search_page_test_url = 'https://fd10-courses.leclercdrive.fr/magasin-159201-Colombes-Michelet/recherche.aspx?TexteRecherche=champagne'
...
def pdct_parsing(fpath, url, products): # TODO : modify xpaths
    ...
    products[url].update({
        'volume': clean_xpathd_text(tree.xpath('//h1[@class="titre-fiche"]//text()')),
        'pdct_img_main_url': clean_url(''.join(tree.xpath('//div[@class="tc-viewport"]//img/@src')), root_url),
        'ctg_denom_txt': ' '.join(tree.xpath('//ul[@class="ulWCAD307_FilAriane"]//text()')),
    })
    return products
...
kw_search_url = "https://fd9-courses.leclercdrive.fr/magasin-056011-Senlis/recherche.aspx?TexteRecherche={kw}"  # TODO : modify URL
for kw in keywords:
    searches[kw] = []
