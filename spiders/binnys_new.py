...
from ers import all_keywords_usa as keywords, fpath_namer, mh_brands, clean_url, headers, shop_inventory_lw_csv
...
def getprice(pricestr):
    ...
    if price is None:
        ...
        if price is None:
            ...
            return price.named['kd'] * 100000 + price.named['dollar'] * 100
        else:
            return price.named['kd'] * 100000 + price.named['dollar'] * 100 + price.named['cent']
    else:
        return price.named['dollar'] * 100 + price.named['cent']


# ##################
# # CTG page xpathing #
# ##################
ctg_page_test_url = 'https://www.binnys.com/spirits?cat=431&p=2&product_list_limit=72'
...
def ctg_parsing(fpath, ctg, categories, products):  # TODO : modify xpaths
    ...
    for li in tree.xpath('//ol[contains(@class, "product-items")]/li'):
        if not li.xpath('(.//a/@href)[1]'):
            continue
        produrl = li.xpath('(.//a/@href)[1]')[0]
        produrl = parse_qs(urlsplit(produrl).query)['url'][0] if 'url' in parse_qs(urlsplit(produrl).query) else produrl
        products[produrl] = {
            'pdct_name_on_eretailer': clean_xpathd_text(li.xpath('.//div[@class="product-list-main"]//text()')),
            'raw_price': clean_xpathd_text(li.xpath('(.//span[@class="price"]//text())[1]')),
            'raw_promo_price': clean_xpathd_text(li.xpath('.//span[contains(@id, "old-price-")]/@data-price-amount')),
            'volume': clean_xpathd_text(li.xpath('.//div[@class="size-pack-attr"]//text()')),
        }
        products[produrl]['brnd'] = brm.find_brand(products[produrl]['pdct_name_on_eretailer'])['brand']
        print(products[produrl], produrl)
        products[produrl]['price'] = getprice(products[produrl]['raw_price'])
        if products[produrl]['raw_promo_price'] and "$" not in products[produrl]['raw_promo_price']:
            products[produrl]['raw_promo_price'] = "$" + products[produrl]['raw_promo_price']
...
# # KW page xpathing #
###################
search_page_test_url = 'https://www.binnys.com/catalogsearch/result/index/?q={kw}&product_list_limit=72'
...
# # PDCT page xpathing #
###################
pdct_page_test_url = 'https://www.binnys.com/moet-chandon-nectar-imperial-rose-256554.html'
...
def pdct_parsing(fpath, url, products): # TODO : modify xpaths
    ...
    products[url].update({
        'pdct_img_main_url': clean_url(''.join(tree.xpath('//div[contains(@class, "fotorama__loaded")]/@href')[:1]), root_url),
    })
    return products
...
urls_ctgs_dict = {
    'vodka': 'https://www.binnys.com/spirits?cat=431&p={page}&product_list_limit=72',
    'sparkling': 'https://www.binnys.com/catalogsearch/result/index/?q=+Sparkling+wine&p={page}&product_list_limit=72',
    'cognac': 'https://www.binnys.com/spirits?cat=429&p={page}&product_list_limit=72',
    'champagne': 'https://www.binnys.com/wine?area=46101&p={page}&product_list_limit=72',
    'still_wines': 'https://www.binnys.com/wine/white-wines/sauvignon-blanc?product_list_limit=72&p={page}',
    'whisky': 'https://www.binnys.com/spirits?cat=433&p={page}&product_list_limit=72',
    'gin': 'https://www.binnys.com/spirits?cat=432&p={page}&product_list_limit=72',
    'tequila': 'https://www.binnys.com/spirits?cat=430&p={page}&product_list_limit=72',
    'rum': 'https://www.binnys.com/spirits?cat=434&p={page}&product_list_limit=72',
    'red_wine': 'https://www.binnys.com/catalogsearch/result/index/?q=+pinot+noir&p={page}&product_list_limit=72',
    'white_wine': 'https://www.binnys.com/wine/white-wines/sauvignon-blanc?product_list_limit=72&p={page}',
    'liquor': 'https://www.binnys.com/spirits?cat=248&p={page}&product_list_limit=72',
    # 'bourbon': '',
}
...
kw_search_url = "https://www.binnys.com/catalogsearch/result/index/?q={kw}&product_list_limit=72"  # TODO : modify URL
for kw in keywords:
    searches[kw] = []
