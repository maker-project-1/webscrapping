...
# # CTG page xpathing #
# ##################
ctg_page_test_url = 'https://www.target.com/c/wine-beer-liquor-beverages-grocery/-/N-5xsxv'
...
def ctg_parsing(fpath, ctg, categories, products):  # TODO : modify xpaths
    ...
    for li in tree.xpath('//ul[@class="h-padding-t-tight Row-fhyc8j-0 gGOZwz"]/li'):
        if not li.xpath('.//a/@href'):
            continue
        produrl = li.xpath('.//a/@href')[0]
        produrl = parse_qs(urlsplit(produrl).query)['url'][0] if 'url' in parse_qs(urlsplit(produrl).query) else produrl
        products[produrl] = {
            'pdct_name_on_eretailer': clean_xpathd_text(li.xpath('.//div[@class="h-display-flex"]/a//text()')),
            'volume': clean_xpathd_text(li.xpath('.//div[@class="h-display-flex"]/a//text()')),
            'raw_price': clean_xpathd_text(li.xpath('./zzzzzzzz')),
            'raw_promo_price': clean_xpathd_text(li.xpath('./zzzzzzzzzzzz')),
            'pdct_img_main_url': "".join(li.xpath('.//picture[@data-test="product-image"]//@srcset')[0]),
        }
...
# # KW page xpathing #
###################
search_page_test_url = 'https://www.target.com/s?searchTerm=champagne'
...
def kw_parsing(fpath, kw, searches, products):  # TODO : modify xpaths
    ...
    for li in tree.xpath('//ul[contains(@id ,"product_list")]/li'):
        ...
        products[produrl] = {
            'pdct_name_on_eretailer': clean_xpathd_text(li.xpath('.//div[@class="h-display-flex"]/a//text()')),
            'volume': clean_xpathd_text(li.xpath('.//div[@class="h-display-flex"]/a//text()')),
            'raw_price': clean_xpathd_text(li.xpath('./zzzzzzzzzz')),
...
def pdct_parsing(fpath, url, products): # TODO : modify xpaths
    ...
    products[url].update({
        'volume': clean_xpathd_text(tree.xpath('//span[@data-test="product-title"]//text()')),
        'pdct_img_main_url': clean_url(''.join(tree.xpath('//div[@style="position: relative;"]//img/@src')[0]), root_url),
        'ctg_denom_txt': ' '.join(tree.xpath('//div[@data-test="breadcrumb"]//text()')),
    })
    return products
...
urls_ctgs_dict = {
    'whisky': 'https://www.target.com/c/types-of-whiskey/-/N-5xsxwZt4tz3xze8v9Zt4tz3xsashqZt4tz3xt5spyZ4u0vZphcf9Zt4tz3xwimruZt4tz3xm0o8dZem8i6Zt4tz3xu7xnwZt4tz3xtnwq6?type=products&lnk=Shopallwhiskey',
    'champagne': 'https://www.target.com/c/champagne-sparkling-wine-beer-liquor-beverages-grocery/-/N-dd9keZ27e2vr1l0o7?type=products&lnk=Champagne',
    #'cognac': '',
    'sparkling': 'https://www.target.com/c/sparkling-wine-wine-beer-liquor-beverages-grocery/-/N-dd9ke?lnk=Sparkling',
    'vodka': 'https://www.target.com/c/spirits-wine-beer-liquor-beverages-food-beverage/-/N-5xsxwZq6u68?type=products&lnk=Vodka',
    #'still_wines': '',
    'gin': 'https://www.target.com/c/spirits-wine-beer-liquor-beverages-food-beverage/gin/-/N-5xsxwZcdubt?type=products&lnk=Gin',
    'tequila': 'https://www.target.com/c/spirits-wine-beer-liquor-beverages-food-beverage/-/N-5xsxwZt4tz3x30sytZt4tz3xj5ugyZt4tz3xtfi56Zt4tz3xh55yxZt4tz3x4uaf9?type=products&lnk=Shopallwhiskey',
    'red_wine': 'https://www.target.com/c/red-wine-wine-beer-liquor-beverages-grocery/-/N-fups0?lnk=Red',
    'white_wine': 'https://www.target.com/c/white-wine-wine-beer-liquor-beverages-grocery/-/N-yskry?lnk=White',
    'rum': 'https://www.target.com/c/spirits-wine-beer-liquor-beverages-food-beverage/rum/-/N-5xsxwZ5cg5c?type=products&lnk=Rum',
    'bourbon': 'https://www.target.com/c/spirits-wine-beer-liquor-beverages-food-beverage/-/N-5xsxwZphcf9Zt4tz3xm0o8d?type=products&lnk=Bourbon',
    'brandy': 'https://www.target.com/c/spirits-wine-beer-liquor-beverages-food-beverage/brandy/-/N-5xsxwZxscyh?type=products&lnk=Brandy',
    'liquor': 'https://www.target.com/c/liquor-wine-beer-beverages-grocery/-/N-5xsxw'
}
...
kw_search_url = "https://www.target.com/s?searchTerm={kw}&Nao={page}"  # TODO : modify URL
for kw in keywords:
    searches[kw] = []
