...
# # CTG page xpathing #
# ##################
ctg_page_test_url = 'https://www.karstadt.de/search?q=champagner&prefn1=Produkttyp&prefv1=Champagner&sz=36'
...
# # KW page xpathing #
###################
search_page_test_url = 'https://www.karstadt.de/search?q=champagner'
...
def kw_parsing(fpath, kw, searches, products):  # TODO : modify xpaths
    ...
    for li in tree.xpath('//ul[contains(@class, "tiles-container")]/li'):
        ...
        products[produrl] = {
            'pdct_name_on_eretailer': clean_xpathd_text(li.xpath('.//span[@class="product-name"]/text()')),
            'volume': clean_xpathd_text(li.xpath('.//div[contains(@class, "price-sales")]/span[last()]/text()')),
            'raw_price': clean_xpathd_text(li.xpath('.//div[contains(@class,"price-sales")]/@data-baseprice')[0] * 100),
...
def pdct_parsing(fpath, url, products): # TODO : modify xpaths
    ...
    products[url].update({
        ...
        'pdct_img_main_url': clean_url(''.join(tree.xpath('//img[@class="primary-image"]/@src')[0]), root_url),
        'ctg_denom_txt': ' '.join(tree.xpath('//ol[@class="breadcrumb"]//a/text()')),
    })
    return products
