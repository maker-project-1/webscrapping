...
shop_id = 'virgin_wines'
root_url = 'https://www.virginwines.co.uk'
country = 'UK'
...
ctg_page_test_url = 'https://www.virginwines.co.uk/search/filter-by/wine-category/bubbly/wine-style/champ?pageSize=96&page=1'
...
def ctg_parsing(fpath, ctg, categories, products):  # TODO : modify xpaths
    ...
    for li in tree.xpath('//div[@class="row product-list"]/div'):
        if not li.xpath('(.//a/@href)[1]'):
            continue
        produrl = li.xpath('(.//a/@href)[1]')[0]
        produrl = parse_qs(urlsplit(produrl).query)['url'][0] if 'url' in parse_qs(urlsplit(produrl).query) else produrl
        products[produrl] = {
            'pdct_name_on_eretailer': clean_xpathd_text(li.xpath('.//*[@data-gacategory="product-name"]//text()')),
            'volume': clean_xpathd_text(li.xpath('.//*[@data-gacategory="product-name"]//text()')),
            'raw_price': clean_xpathd_text(li.xpath('.//span[@class="d-block price text-right"]//text()')),
            'raw_promo_price': clean_xpathd_text(li.xpath('.//span/del/text()')),
            'pdct_img_main_url': "".join(li.xpath('.//img/@data-original')[:1]),
        }
        products[produrl]['brnd'] = brm.find_brand(products[produrl]['pdct_name_on_eretailer'])['brand']
        print(products[produrl], produrl)
        products[produrl]['price'] = getprice(products[produrl]['raw_price'])
        products[produrl]['promo_price'] = getprice(products[produrl]['raw_promo_price'])
        products[produrl]['pdct_img_main_url'] = clean_url(products[produrl]['pdct_img_main_url'], root_url).replace('/90/', '/400/').replace('.jpg', '.png').replace('.jpeg', '.png')
...
# # KW page xpathing #
###################
search_page_test_url = 'https://www.virginwines.co.uk/search/{kw}?pageSize=96&page=1'
...
def kw_parsing(fpath, kw, searches, products):  # TODO : modify xpaths
    ...
    for li in tree.xpath('//div[@class="row product-list"]/div'):
        ...
        products[produrl]['price'] = getprice(products[produrl]['raw_price'])
        products[produrl]['promo_price'] = getprice(products[produrl]['raw_promo_price'])
        products[produrl]['pdct_img_main_url'] = clean_url(products[produrl]['pdct_img_main_url'], root_url).replace(
            '/90/', '/400/').replace('.jpg', '.png').replace('.jpeg', '.png')
        print(products[produrl])
...
urls_ctgs_dict = {
    'champagne': 'https://www.virginwines.co.uk/search/filter-by/wine-category/bubbly/wine-style/champ?pageSize=96&page={page}',
    'sparkling': 'https://www.virginwines.co.uk/search/filter-by/wine-category/bubbly/wine-style/champ/wine-style/spark?pageSize=96&page={page}',
    'still_wines': 'https://www.virginwines.co.uk/browse/filter-by/wine-category/white-wine?pageSize=96&page={page}',
    'red_wine': 'https://www.virginwines.co.uk/browse/filter-by/wine-category/red-wine?pageSize=96&page={page}',
    'white_wine': 'https://www.virginwines.co.uk/browse/filter-by/wine-category/red-wine?pageSize=96&page={page}',
    'liquor': 'https://www.virginwines.co.uk/browse/filter-by/wine-category/sherry?pageSize=96&page={page}',
}
