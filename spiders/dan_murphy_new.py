...
shop_id = 'dan_murphy'
root_url = 'https://danmurphys.com.au'
...
# ##################
ctg_page_test_url = 'https://www.danmurphys.com.au/champagne-sparkling'
...
def ctg_parsing(fpath, ctg, categories, products):  # TODO : modify xpaths
    ...
    for li in tree.xpath('//ul[@class="row product-list"]/li'):
        if not li.xpath('(.//a/@href)[1]'):
            continue
        if 'clearance' in li.xpath('(.//a/@href)[1]'):
            continue
        produrl = li.xpath('(.//a/@href)[1]')[0]
        produrl = parse_qs(urlsplit(produrl).query)['url'][0] if 'url' in parse_qs(urlsplit(produrl).query) else produrl
        products[produrl] = {
            'pdct_name_on_eretailer': clean_xpathd_text(li.xpath('.//h2//a//text()')),
            'volume': clean_xpathd_text(li.xpath('./zzzzz')),
            'raw_price': clean_xpathd_text(li.xpath('.//div[@itemprop="price"]//text()')[:1]),
            'raw_promo_price': clean_xpathd_text(li.xpath('.//span[contains(@id, "old-price-")]/@data-price-amount')),
            'pdct_img_main_url': "".join(li.xpath('.//shop-image/div/img/@src')[:1]),
        }
...
# # KW page xpathing #
###################
search_page_test_url = 'https://www.danmurphys.com.au/search?searchTerm={kw}'
exple_kw_page_path = op.join(TEST_PAGES_FOLDER_PATH, shop_id, 'kw_page_test.html') #TODO : store the file
os.makedirs(op.dirname(exple_ctg_page_path), exist_ok=True)
kw_test, test_searches, test_products = 'chat', {"chat": []}, {}
...
def pdct_parsing(fpath, url, products): # TODO : modify xpaths
    ...
    products[url].update({
        'volume': clean_xpathd_text(tree.xpath('//ul[@class="col-md-6 product-attributes"]//li//text()')),
        'pdct_img_main_url': clean_url(''.join(tree.xpath('//img[@alt="product image"]/@src')[:1]), root_url),
        'ctg_denom_txt': ' '.join(tree.xpath('//breadcrumbs//text()')),
    })
    return products
...
kw_search_url = "https://www.danmurphys.com.au/search?searchTerm={kw}"  # TODO : modify URL
for kw in keywords:
    searches[kw] = []
