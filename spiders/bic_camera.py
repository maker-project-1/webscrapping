...
# # CTG page xpathing #
###################
exple_ctg_page_path = op.join(TEST_PAGES_FOLDER_PATH, "bic_camera", 'ctg_page_test.html') # TODO : store the file
...
def ctg_parsing(fpath, ctg, categories, products):  # TODO : modify xpaths
    ...
    for li in tree.xpath('//div[@class="bcs_listItem"]/ul/li'):
        ...
        products[produrl] = {
            'pdct_name_on_eretailer': clean_xpathd_text(li.xpath('.//p[@class="bcs_title"]/a//text()'), unicodedata_normalize=True),
            'raw_price': clean_xpathd_text(li.xpath('.//p[@class="bcs_price"]//text()'), unicodedata_normalize=True),
            'raw_promo_price': clean_xpathd_text(li.xpath('.//xpath/text()'), unicodedata_normalize=True),
            'volume': clean_xpathd_text(li.xpath('.//p[@class="bcs_title"]/a//text()'), unicodedata_normalize=True),
            'pdct_img_main_url': clean_url(li.xpath('.//p[@class="bcs_image"]//img/@src')[0], root_url),
        }
        print(products[produrl], produrl)
        products[produrl]['price'] = getprice(products[produrl]['raw_price'])
        products[produrl]['promo_price'] = getprice(products[produrl]['raw_promo_price'])
        products[produrl]['pdct_img_main_url'] = products[produrl]['pdct_img_main_url'].split("?")[0] + "?sr.dw=320&sr.jqh=60&sr.dh=320&sr.mat=1"
...
exple_kw_page_path = op.join(TEST_PAGES_FOLDER_PATH, "bic_camera", 'kw_page_test.html') # TODO : store the file
...
def kw_parsing(fpath, kw, searches, products):  # TODO : modify xpaths
    ...
    for li in tree.xpath('//div[@class="bcs_listItem"]/ul/li'):
        ...
        products[produrl] = {
            ...
            'raw_price': clean_xpathd_text(li.xpath('.//p[@class="bcs_price"]//text()'), unicodedata_normalize=True),
            'raw_promo_price': clean_xpathd_text(li.xpath('.//xpath/text()'), unicodedata_normalize=True),
            'volume': clean_xpathd_text(li.xpath('.//p[@class="bcs_price"]//text()'), unicodedata_normalize=True),
            'pdct_img_main_url': clean_url(li.xpath('.//p[@class="bcs_image"]//img/@src')[0], root_url),
        }
        print(products[produrl], produrl)
        products[produrl]['price'] = getprice(products[produrl]['raw_price'])
